...
from `rd+rs` — and the two disagree by **2.43×** on NDMOS20V and **0.08×** on
NDMOS200V, a 30× swing across the family. Is that disagreement real information
...
|---|---|---|
| NDMOS20V | 2.310 | **1.610** |
| NDMOS200V | 44.744 | **27.225** |

**Idsat density (mA/µm)**

| device | stock | rd=rs=0 |
|---|---|---|
| NDMOS20V | 1674.4 | 2074.5 |
| NDMOS200V | 117.7 | 166.1 |

### The decomposition is clean

Subtracting the `rd=rs=0` on-resistance from the stock one recovers the card's
own `rd+rs` to **0.03 %** (NDMOS20V: 0.07002 Ω measured vs 0.070 carded) and
**0.11 %** (NDMOS200V: 1.7520 vs 1.750). Ron really does separate into a channel
term and a series term, so the split below can be trusted.

| device | Ron stock | channel | series | **series share** |
|---|---|---|---|---|
| NDMOS20V | 0.2310 Ω | 0.1610 Ω | 0.0700 Ω | **30.3 %** |
| NDMOS200V | 4.4744 Ω | 2.7225 Ω | 1.7520 Ω | **39.2 %** |
...
**1.239×** (NDMOS20V) and **1.411×** (NDMOS200V), against implied-width gaps of
...
|---|---|---|---|---|---|
| NDMOS20V | 3649× | 951× | **288×** | 2.43× | **12.66×** |
| NDMOS200V | 287× | 2327× | **911×** | 0.08× | **0.31×** |
...
> **NDMOS20V = 1.610 Ω·µm, NDMOS200V = 27.22 Ω·µm** at `Vov = 4 V`, `Vds = 0.1 V`.
...
`../../results/local_models/{NDMOS20V,NDMOS200V}_d3.mod`.
