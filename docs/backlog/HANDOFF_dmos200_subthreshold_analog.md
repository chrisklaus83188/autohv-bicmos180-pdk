# PDK handoff: `NDMOS200V`/`PDMOS200V` are subthreshold at analog (µA) currents — power-FET `kp`

**To:** AutoHV-BiCMOS180 PDK maintainer
**From:** chuba14f task author (high-side rail-threshold monitor)
**Date:** 2026-06-06
**Models:** `NDMOS200V_INT` / `PDMOS200V_INT` (the `kp` / `vto` stat params).
...
## TL;DR
The 200 V HV DMOS are modeled with **power-FET transconductance** (`KP_PDMOS200V = 0.088 A/V²`,
`KP_NDMOS200V = 0.22 A/V²`). As a result they sit in **subthreshold** at the µA currents an analog HV
...
`PDMOS200V`, W=30 µm L=5 µm, as a self-biased diode at Vds = 200 V (the front-end's operating CM):
```
...
```
(`VTO_PDMOS200V = -1.31 V`.) gm/I stays pinned near the subthreshold ceiling (~26/V ≈ 1/(kT/q·n))
until ~mA. A normal analog MOSFET at this W would be at Vov ≈ +0.2 V (strong inversion) by ~50 µA.

## Why it matters
My front-end is a floating HV current mirror (`PDMOS200V` M_ref/M_mir) that senses CP−VIN at ~tens of
...
## The ask
Is `KP_NDMOS200V`/`KP_PDMOS200V` representative of an **HV drift MOSFET used as an analog device**, or is
...
2. **Confirm these are power-only** and the PDK needs a separate HV *analog* device for µA-class
   front-ends (none currently exists above 5 V — the core MOS top out at `NMOS5V0`/`PMOS5V0`).
...
```spice
.title PDMOS200V subthreshold-at-uA reproducer
.include "autohv_bicmos180_case.lib"
.param case=0 PROC_ON=0 MM_ON=0
V_s  vs 0 DC 200
X_M  vg vg vs PDMOS200V W=30u L=5u    ; diode: d=g=vg, s=vs
