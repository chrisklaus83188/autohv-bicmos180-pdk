...
NMOS1V8 (bench 1.3 µm) 9.64 → 9.24 %, PMOS1V8 11.96 → 11.72, NMOS3V3 7.22 → 7.03, NMOS5V0 6.80 → 6.68,
...
|---|---|---|---|---|
| NMOS1V8 / PMOS1V8 | 7.18 / 8.13 mV | **−1.60 / −1.59** | 8.78 / 9.72 | 1.22 / 1.20 |
| NMOS3V3 / PMOS3V3 | 7.78 / 8.85 | −0.98 / −0.96 | 8.77 / 9.81 | 1.13 / 1.11 |
| NMOS5V0 / PMOS5V0 | 8.34 / 9.65 | −1.17 / −1.12 | 9.51 / 10.77 | 1.14 / 1.12 |
| NMOS12V / PMOS12V | 18.79 / 20.89 | −1.90 / −1.92 | 20.69 / 22.81 | 1.10 / 1.09 |
...
drive and slope changes into what they call threshold. The same NMOS1V8 device read −0.18 mV at a
...
An L-sweep on NMOS5V0 settles the mechanism: as carded the residual is −1.171 mV at L = 0.5 µm,
...
`Z_*` params must be named `Z_<exact corners.json key>` — `Z_TOX_18`, `Z_VTH_NMOS1V8`,
`Z_RDSW_NMOS1V8`. The brief's `Z_VTHN18` is illustrative shorthand; `corners.json` is what sets
...
|---|---|---|---|
| NDMOS20V | +2.2595 | −1.7814 | **−0.79** |
| NDMOS60V | +1.2259 | −2.6517 | −2.16 |
| NDMOS80V | +0.8904 | −2.8110 | −3.16 |
| NDMOS120V | +0.5796 | −2.9163 | −5.03 |
| NDMOS200V | +0.4350 | −2.9505 | **−6.78** |
...
- **`σ(KP) = 0` where drift-dominated is measured and implementable** — the five devices are
  identified by their own `U0` slope against the harness's 0.4 floor: NDMOS80V (0.365),
  NDMOS120V (0.265), PDMOS120V (0.285), NDMOS200V (0.212), PDMOS200V (0.216).
...
Also worth noting for the 80 V pair: NDMOS80V at 0.365 is drift-dominated while PDMOS80V at 0.412 is
