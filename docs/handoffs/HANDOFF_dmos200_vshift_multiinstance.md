...
**Models affected:** `NDMOS200V`, `PDMOS200V` (`autohv_bicmos180_case.lib`).
...
(each = diode-connected `PDMOS200V` mirror + `NDMOS200V` cascode, floating at VIN up to 160–200 V).
...
`PDMOS200V`/`NDMOS200V` in a floating front-end adds a `vshift#branch` unknown — the diode mirror
...
| Config | Result |
|---|---|
| 1 `PDMOS200V` diode+mirror, drain loaded 50 kΩ to gnd | converges (device alone is fine at 165 V Vds) |
...
```spice
.title PDMOS200V multi-instance floating-mirror convergence repro
...
X_MrefB refgB refgB vcpB PDMOS200V W=30u L=5u
X_RrefB refgB vin   RPOLY_HI W=1u L=90u
X_MmirB mirdB refgB vcpB PDMOS200V W=30u L=5u
R_pullB mirdB 0 100k
X_McascB mirdB vcasc voutB NDMOS200V W=60u L=5u
...
removes the `vshift#branch` matrix unknown from *every* `NDMOS200V`/`PDMOS200V`, so no number of
