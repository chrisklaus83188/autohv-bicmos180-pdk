# PDK handoff: `PDMOS200V` breakdown is below its 200 V class (and below `NDMOS200V`)

**Models affected:** `PDMOS200V` (and a sanity question on `NDMOS200V`) — the `PDMOS200V_INT` /
`NDMOS200V_INT` `bv=` lines in `autohv_bicmos180_case_models.inc`.
...
## TL;DR
`PDMOS200V` avalanche breakdown at the **FF/SF** corners is **194.58 V** — *below* 200 V, and below
`NDMOS200V`'s worst corner (211.5 V). A high-side circuit that puts the full rail across one HV
device therefore avalanches at FF when VIN ≳ 195 V. Heads-up + a request to confirm whether
`PDMOS200V` is meant to be a true 200 V part. **No action is strictly required for my task** (I'm
...
NDMOS200V  225 V   211.5 V  238.5 V  211.5 V  238.5 V   211.5  (FF/FS)
PDMOS200V  207 V   194.58V  219.42V  219.42V  194.58V   194.58 (FF/SF)   <-- below 200 V
```
...
V_casc vcasc 0 DC 5
X_Mref refg refg vhigh PDMOS200V W=30u L=5u
X_Rref refg vlow  RPOLY_HI W=1u L=90u
X_Mmir mird refg vhigh PDMOS200V W=30u L=5u
R_pull mird 0 100k
X_Mcasc mird vcasc vout NDMOS200V W=60u L=5u
...
1. **Is `PDMOS200V` intended to be a true 200 V device?** If yes, raise FF/SF `bv` to give margin
...
2. **Is 194.58 V the correct silicon?** p-LDMOS often *do* break down lower than the n-side, so it
   may be right — if so, please confirm and note in the reference manual that `PDMOS200V` is a
   ~190 V-class part so downstream HV designs cap VIN accordingly.
3. **The N/P asymmetry in which corners are weak** (`NDMOS200V` weak at FF & FS, `PDMOS200V` at FF &
...
you re-rate `PDMOS200V` to a true 200 V part, ping me and I can raise VIN back toward 200 V (the task
