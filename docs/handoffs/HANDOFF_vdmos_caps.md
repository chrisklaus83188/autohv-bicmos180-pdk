...
the `W_REF=10µm` reference cell. A 40µm-wide NDMOS200V (a typical cascode size)
...
|---|---|---|---|
| NDMOS20V_INT (839)  | 4.032e-10 | 4.992e-10 | 1.4e-10 |
| PDMOS20V_INT (865)  | 3.456e-10 | 4.032e-10 | 1.5e-10 |
| NDMOS40V_INT (995)  | 2.52e-10  | 3.36e-10  | 1.0e-10 |
| PDMOS40V_INT (1021) | 1.92e-10  | 2.52e-10  | 1.05e-10 |
| NDMOS60V_INT (891)  | 1.536e-10 | 2.112e-10 | 7.5e-11 |
| PDMOS60V_INT (917)  | 1.152e-10 | 1.44e-10  | 6.5e-11 |
| NDMOS80V_INT (1047) | 1.0e-10   | 1.35e-10  | 5.5e-11 |
| PDMOS80V_INT (1073) | 7.5e-11   | 9.5e-11   | 4.5e-11 |
| NDMOS120V_INT (943) | 6.24e-11  | 8.64e-11  | 3.5e-11 |
| PDMOS120V_INT (969) | 4.7e-11   | 6.1e-11   | 2.9e-11 |
| NDMOS200V_INT (1099)| 3.5e-11   | 4.8e-11   | 2.2e-11 |
| PDMOS200V_INT (1125)| 2.6e-11   | 3.4e-11   | 1.8e-11 |
| DNMOS20V_INT (1151) | 1.44e-10  | 2.016e-10 | 7e-11 |

For NDMOS200V_INT the exact lines are: cgdmax@1108, cgdmin@1109, cgs@1111,
cjo@1112.
...
`NMOS5V0` BSIM model in the same PDK uses overlap densities of ~0.2 fF/µm
(`cgso≈2e-10 F/m`) — i.e. the VDMOS cards are ~10,000× denser than the LV
devices. A 200µm-wide NMOS5V0 (5× wider than the cascode in question) has only
...
device in the physical range (NDMOS200V: cgs 48fF, cjo 22fF; NDMOS20V: cgs 0.5pF,
...
```spice
* coss_check.spice — terminal C at the drain of a 40um NDMOS200V, channel off
...
XN5 D5 VDD S5  NDMOS200V W=40u L=8u    ; cascode, gate=VDD
XN7 S5 ON  0   0 NMOS5V0  W=200u L=1u  ; pulldown, gate held LOW (off)
...
`cjo→0` in NDMOS200V_INT collapses it to **3.96V** (= the intended `VDD−Vth`),
...
2. Verify with Repro 1: AC drain capacitance of a 40µm NDMOS200V should be in
