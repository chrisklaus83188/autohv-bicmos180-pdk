...
def ngspice_version() -> str:
    """Actual `ngspice --version` banner token (e.g. 'ngspice-45'), stamped into the
    results JSON (ground rule 3)."""
    global _NGVER
    if _NGVER:
        return _NGVER
    _NGVER = "unknown"
...
def main():
    ...
    payload = {"_provenance": provenance({"case": args.case, "mc_n": mc_n,
                                          "vsup": VSUP}),
               "results": rows}
