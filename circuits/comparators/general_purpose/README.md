...
referred 1σ offset, Monte Carlo (run count in the table below). Iq = worst case over both output states.

<!-- AUTOGEN:cmp_variant subdir=general_purpose -->
| Variant | In | role | Iq (µA) | Vos_σ (mV) | Gain (dB) | tpd ↑/↓ (ns) | Hyst (mV) | Area (µm²) |
|---|---|---|---|---|---|---|---|---|
| `nin_gp` | NMOS | normal | 40.9 | 4.11 | 92 | 64 / 23 | — | 143 |
| `nin_lo` | NMOS | lower offset | 40.9 | 2.15 | 98 | 75 / 35 | — | 323 |
| `nin_lo2` | NMOS | lowest offset | 40.9 | 1.36 | 99 | 86 / 49 | — | 623 |
| `nin_hyst` | NMOS | hysteresis | 41.2 | 4.15 | 100 | 71 / 25 | 10.1 | 148 |
| `nin_lp` | NMOS | low power | 8.9 | 4.79 | 95 | 258 / 68 | — | 143 |
| `nin_fast` | NMOS | fast | 79.0 | 4.31 | 90 | 38 / 17 | — | 143 |
| `pin_gp` | PMOS | normal | 38.3 | 5.39 | 93 | 19 / 49 | — | 188 |
| `pin_lo` | PMOS | lower offset | 38.3 | 2.62 | 98 | 32 / 63 | — | 458 |
| `pin_lo2` | PMOS | lowest offset | 38.3 | 1.47 | 97 | 46 / 78 | — | 908 |
| `pin_hyst` | PMOS | hysteresis | 38.7 | 5.45 | 100 | 23 / 60 | 12.7 | 200 |
| `pin_lp` | PMOS | low power | 8.0 | 6.16 | 94 | 55 / 189 | — | 188 |
| `pin_fast` | PMOS | fast | 75.2 | 5.39 | 90 | 14 / 30 | — | 188 |

<sub>Generated by `report_refresh.py` from `comparator_results.json` · models **v2-grounded** · **ngspice-45** · MC N=200.</sub>
<!-- /AUTOGEN:cmp_variant -->

`Vos_σ` is input-referred 1σ from Monte Carlo (basis as stated in the table above). The
...
<!-- AUTOGEN:cmp_fin subdir=general_purpose -->
| `FIN` | role | NIN: Vos_σ / area | PIN: Vos_σ / area | gain |
|---|---|---|---|---|
| 1 | `gp` (normal) | 4.11 mV / 143 µm² | 5.39 mV / 188 µm² | 92–93 dB |
| 2 | `lo` (lower) | 2.15 mV / 323 µm² | 2.62 mV / 458 µm² | 98–98 dB |
| 3 | `lo2` (lowest) | 1.36 mV / 623 µm² | 1.47 mV / 908 µm² | 97–99 dB |

<sub>Generated by `report_refresh.py` from `comparator_results.json` · models **v2-grounded** · **ngspice-45** · MC N=200.</sub>
<!-- /AUTOGEN:cmp_fin -->

(FIN 1–3 are measured on the Monte-Carlo basis stated in the table; FIN=4 is an
unshipped illustration of the knob at ~0.45 mV NIN / ~0.55 mV PIN, ~1042/1538 µm².)
