...
VARIANTS = {
    ...
    "nin_gp":   dict(sub="CMP_NIN", cm=3.0, IREF=5e-6,  WSCALE=0.5, WIN=20e-6, LIN=1e-6, LANA=1e-6, FIN=1, HYSK=0),    # normal / low-area
    "nin_lo":   dict(sub="CMP_NIN", cm=3.0, IREF=5e-6,  WSCALE=0.5, WIN=20e-6, LIN=1e-6, LANA=1e-6, FIN=2, HYSK=0),    # lower offset
...
def dc_deck(v: dict, case: int) -> str:
    return (header(case, 0, 0) +
            ...
            "  meas dc vtrip   when v(out)=2.5\n"
            "  meas dc voh     max v(out)\n"
            "  meas dc vol     min v(out)\n"
            "  meas dc vin_lo  when v(out)=0.5\n"
            "  meas dc vin_hi  when v(out)=4.5\n"
            ".endc\n.end\n")
...
def tran_deck(v: dict, case: int) -> str:
    ...
    return (header(case, 0, 0) +
            ...
            ".control\n"
            "  tran 0.5n 5u\n"
            f"  meas tran tpd_lh TRIG v(inp) VAL={cm} RISE=1 TARG v(out) VAL=2.5 RISE=1\n"
            f"  meas tran tpd_hl TRIG v(inp) VAL={cm} FALL=1 TARG v(out) VAL=2.5 FALL=1\n"
            ".endc\n.end\n")
...
def hyst_deck(v: dict, case: int) -> str:
    ...
    return (header(case, 0, 0) +
            ...
            ".control\n"
            "  tran 20n 100u\n"
            "  meas tran vtrip_up when v(out)=2.5 RISE=1\n"
            "  meas tran vtrip_dn when v(out)=2.5 FALL=1\n"
            "  meas tran vin_up   find v(inp) when v(out)=2.5 RISE=1\n"
            "  meas tran vin_dn   find v(inp) when v(out)=2.5 FALL=1\n"
            ".endc\n.end\n")
