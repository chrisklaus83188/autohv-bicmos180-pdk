...
rule of 1.4, same as the GP 1.8 V family (at 1.8 V, |Vth| ≈ ⅓ of the rail). Under
`v2-grounded` the RR always-on devices in fact clear the stricter 1.4 rule at every
corner including brown-out (see the fenced table) — the rail-to-rail front end holds
more headroom than a single-pair stage, so the 1.1 relaxation is unused margin here.

`python run_saturation.py --pvt` — every always-on device holds **> 1.1 over the
full 0→1.8 V CM, all corners, −40/+125 °C, 1.62–1.98 V**:

<!-- AUTOGEN:rr_sat subdir=rail_to_rail_1v8 -->
| VDD | always-on min Vds/Vdsat | binding corner | rule |
|---|---|---|---|
| 1.62 V | 1.72 | SS / +125 C | >1.1 |
| 1.8 V | 2.27 | SS / -40 C | >1.1 |
| 1.98 V | 2.96 | FF / +125 C | >1.1 |
