...
w("# Monte Carlo check + NMOS5V0 current-mirror mismatch")
w()
...
w("## 2. The mirror")
w()
w("Simple two-transistor NMOS mirror, nothing cascoded. Both devices `NMOS5V0`,")
w("W = %g um, L = %g um, which is the sizing guide's own gm/Id ~ 6 entry for" % (M.W, M.L))
w("NMOS5V0 at 10 uA. Reference is an ideal 10 uA source into the diode-connected")
w("device; the output sits at Vdd/2.")
w()
w("```spice")
w("Vdd  dd 0 %g" % M.VDD)
w("Iref dd in %gu" % (M.IREF * 1e6))
w("X1 in  in 0 0 NMOS5V0 W=%gu L=%gu    ; diode-connected reference" % (M.W, M.L))
w("X2 out in 0 0 NMOS5V0 W=%gu L=%gu    ; mirror output" % (M.W, M.L))
...
w("formula says it should be before any simulation is run. `NMOS5V0` in")
