...
| DLY  | two-sided delay: BOTH edges 20 ns (no bypass) |
...
| DLY* | 20.1 ns / 14-29 ns / 56 um^2 | 19.9 ns / 15-26 ns / 58 um^2 | 19.8 ns / 16-29 ns / 58 um^2 |

<sub>Each cell: nominal delay/width / full-PVT min-max / active area. *DLY shows the rising edge; the falling edge is a matched ~20 ns delay (both-edge detail in CHARACTERIZATION.md).</sub>

## Key points
- 20 ns target met at nominal for all 12 edge-asymmetric cells (19.7-20.4 ns); the two-sided DLY hits ~20 ns on BOTH edges (see CHARACTERIZATION.md).
