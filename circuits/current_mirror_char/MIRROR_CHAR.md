...
<!-- AUTOGEN:mirror_dc -->
| topology (B_10u, TT, 27 °C) | gain @Vdd/2 | λ_eff (/V) | r_out (MΩ) | ramp nonlin | compliance V_out,max (1 %) | area (µm²) |
|---|---|---|---|---|---|---|
| MIR_S (simple) | 1.0278 | 2.20e-02 | 4.3 | 2.57 % | 0.31 V | 325 |
| MIR_CS (std cascode) | 1.0000 | 2.33e-05 | 4290.5 | 0.00 % | 4.79 V | 650 |
| MIR_CW (wide-swing) | 1.0002 | 1.18e-04 | 847.8 | 0.01 % | 5.20 V | 650 |

<sub>Generated by `report_refresh.py` from `metrics.csv` · models **v2-grounded** · **ngspice-45**.</sub>
<!-- /AUTOGEN:mirror_dc -->
...
## 6. Monte Carlo — mismatch dominates the ratio, process moves λ  (`mc_results.json`)

Nominal PVT, two separate modes (mismatch-only = MM_ON; process+mismatch = MM_ON &
PROC_ON), on the full V_out grid (run count in the table); σ/µ of I_out at Vdd/2:

<!-- AUTOGEN:mirror_mc -->
| design | topo | mode | gain | σ/µ (%) | λ σ |
|---|---|---|---|---|---|
| B_10u | MIR_S | mismatch | 1.0218 | 1.256 | 2.04e-05 |
| B_10u | MIR_S | procmm | 1.0219 | 1.344 | 5.10e-04 |
| B_10u | MIR_CS | mismatch | 0.9990 | 1.346 | 9.80e-08 |
| B_10u | MIR_CS | procmm | 1.0000 | 1.229 | 9.60e-07 |
| B_10u | MIR_CW | mismatch | 1.0006 | 1.332 | 3.51e-06 |
| B_10u | MIR_CW | procmm | 0.9986 | 1.161 | 1.72e-05 |
| B_100n | MIR_S | mismatch | 1.0099 | 14.308 | 2.20e-04 |
| B_100n | MIR_S | procmm | 1.0118 | 12.362 | 5.30e-04 |
| B_100n | MIR_CS | mismatch | 0.9993 | 12.491 | 5.42e-07 |
| B_100n | MIR_CS | procmm | 0.9992 | 12.989 | 1.11e-06 |
| B_100n | MIR_CW | mismatch | 0.9859 | 11.497 | 8.89e-06 |
| B_100n | MIR_CW | procmm | 1.0219 | 11.165 | 1.04e-05 |
| A_100n | MIR_S | mismatch | 1.0348 | 2.463 | 5.29e-06 |
| A_100n | MIR_S | procmm | 1.0317 | 2.563 | 6.07e-04 |
| A_100n | MIR_CS | mismatch | 1.0008 | 2.733 | 2.33e-07 |
| A_100n | MIR_CS | procmm | 1.0015 | 2.412 | 9.62e-07 |
| A_100n | MIR_CW | mismatch | 1.0011 | 2.454 | 2.04e-07 |
| A_100n | MIR_CW | procmm | 1.0003 | 2.677 | 9.92e-07 |

<sub>Generated by `report_refresh.py` from `mc_results.json` · models **v2-grounded** · **ngspice-45** · MC N=200.</sub>
<!-- /AUTOGEN:mirror_mc -->
...
mc_results.json  MC runs (3 designs × 3 topo × 2 modes × N; N in the §6 table), every run kept
